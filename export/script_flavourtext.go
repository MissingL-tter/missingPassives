...
func init() {
	Scripts = append(Scripts, Script{Name: "flavourText", Outs: []string{"Data/FlavourText.lua"}, Run: scriptFlavourText})
}
...
func scriptFlavourText(x *Ctx) error {
	...
	out := x.Out("Data/FlavourText.lua")
	out.W("-- This file is automatically generated, do not edit!\n")
	out.W("-- Flavour text data (c) Grinding Gear Games\n\n")
	out.W("return {\n")

	index := 1
	x.flavourEntries = nil
	writeEntry := func(id, name string, textLines []string) {
	...
	for _, entry := range forcedNameList {
		if textLines, ok := flavourTextById[entry.id]; ok {
			writeEntry(entry.id, entry.name, textLines)
		}
	}
	...
	for _, id := range ids {
		if lines, ok := flavourTextById[id]; ok {
			writeEntry(id, uniqueNameLookup[id], lines)
		}
	}
