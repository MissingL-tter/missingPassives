...
func init() {
	Scripts = append(Scripts, Script{Name: "enchant", Outs: []string{
		"Data/EnchantmentBoots.lua", "Data/EnchantmentGloves.lua", "Data/EnchantmentBelt.lua",
		"Data/EnchantmentFlask.lua", "Data/EnchantmentBody.lua", "Data/EnchantmentWeapon.lua",
...
func scriptEnchant(x *Ctx) error {
	x.LoadStatFile("stat_descriptions.txt")

	writeByDiff := func(fileName string, byDiff map[string][][]string) error {
		out := x.Out(strings.TrimPrefix(fileName, "../"))
		...
		for _, diff := range enchantSourceOrder {
			if list, ok := byDiff[diff]; ok {
				out.W("\t[\"" + diff + "\"] = {\n")
				for _, stats := range list {
					out.W("\t\t\"" + strings.Join(stats, "/") + "\",\n")
				}
				out.W("\t},\n")
			}
		}
	...
	doLabEnchantment := func(fileName, group string) error {
		...
		return writeByDiff(fileName, byDiff)
	}
	if err := doLabEnchantment("../Data/EnchantmentBoots.lua", "ConditionalBuffEnchantment"); err != nil {
		return err
	}
	if err := doLabEnchantment("../Data/EnchantmentGloves.lua", "TriggerEnchantment"); err != nil {
		return err
	}
	if err := doLabEnchantment("../Data/EnchantmentBelt.lua", "BuffEnchantment"); err != nil {
		return err
	}

	doOtherEnchantment := func(fileName string, groupsList map[int64]map[string]string) error {
		byDiff := map[string][][]string{}
	...
	if err := doOtherEnchantment("../Data/EnchantmentFlask.lua", map[int64]map[string]string{
		21: {"FlaskEnchantment": "ENKINDLING"},
		22: {"FlaskEnchantment": "INSTILLING"},
	}); err != nil {
		return err
	}
	if err := doOtherEnchantment("../Data/EnchantmentBody.lua", map[int64]map[string]string{
		3: {"AlternateArmourQuality": "HARVEST", "EnchantmentHeistArmour": "HEIST"},
	}); err != nil {
		return err
	}
	if err := doOtherEnchantment("../Data/EnchantmentWeapon.lua", map[int64]map[string]string{
	...
	skillOrder := make([]string, 0, len(bySkill))
	for s := range bySkill {
		skillOrder = append(skillOrder, s)
	}
	sort.Strings(skillOrder)
	out := x.Out("Data/EnchantmentHelmet.lua")
	out.W("-- This file is automatically generated, do not edit!\n")
	out.W("-- Item data (c) Grinding Gear Games\n\nreturn {\n")
	for _, skill := range skillOrder {
		out.W("\t[\"" + skill + "\"] = {\n")
		for _, diff := range enchantSourceOrder {
			if list, ok := bySkill[skill][diff]; ok {
				out.W("\t\t[\"" + diff + "\"] = {\n")
				for _, stats := range list {
					out.W("\t\t\t\"" + strings.Join(stats, "/") + "\",\n")
				}
