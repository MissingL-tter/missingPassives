...
| `logic:[~] view:[ ]` | `config` | `Classes/ConfigTab.lua` `Modules/ConfigOptions.lua` `ConfigVisibility.lua` `Classes/ConfigSetListControl.lua` | 4,179 | 580 options (check 331, count 163, countAllowZero 33, list 41, integer 11, float 1) carrying 532 embedded apply closures. **logic**: package `config` ports the load half and `BuildModList` — the option table converted once from the booted reference (`Options`, in varList order, which is load-bearing), `NewConfigSet`'s defaults, `Load`'s `<Input>`/`<Placeholder>`/`<ConfigSet>`/`<CustomModifierBlock>` handling with the `enemyIsBoss` and `presetBossSkills` value migrations, `UpdateLevel`, the custom-modifier parse, and the three control operations the apply functions reach through (`SetPlaceholder`, `SelByValue`, `selIndex`). 36 of 532 apply bodies ported — every option carrying a default, so each fires on every build. **archive**: `test/config_test.go` — `TestConfigStateAgainstReference` compares the loaded input and placeholder tables key for key: 2,667 values across 47 builds, zero disagreements. `TestConfigModListCoverage` compares the modifier lists: 2,294 of 2,347 reproduced, **zero produced that the archive does not have**, 49 unmatched across 32 modifier names — all from options only a few corpus builds set. **Traps**: `SetPlaceholder(v, true)` round-trips through the control's `tostring`/`tonumber`, so a stored placeholder is quantized (`util.FormatG14`); `SetPlaceholder("", true)` DELETES the key; a string-valued `<Placeholder>` is stored as an *input*. `ConfigVisibility.lua` computes per-option relevance from mainEnv usage maps — logic. Widget rendering is view. **This is now the single blocker on running a saved build faithfully**: `build.Load` assembles everything else, but the four config fields (`configInput`, `configPlaceholder`, `configModList`, `configEnemyModList`) come from here. They are not optional — each option's apply closure runs on its *default* as well as on a user selection, so a build whose XML sets two options still gets ~32 Config-sourced mods, and the corpus range is 31–48 on every build. The placeholder half is computed as well (enemy armour, evasion, resistances, damage, scaled to the enemy level). A build computed without them therefore agrees with the application on what config does not touch (attributes, life, cast speed, crit chance) and runs high on damage, the enemy's resistances being among the missing mods. |
