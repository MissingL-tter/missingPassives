...
- **code** — the module is *written*: every branch of the reference exists in
  ...
  belongs here, including its bugs (reproduced faithfully, inventoried in
  `later.md` §2).
...
coverage is high (193 variants agreeing on every checkpoint) while
...
|---|---|---|---|---|---|
| `[~]` | `[~]` | `calc-core` | `Modules/Calcs.lua` `CalcSetup.lua` `CalcPerform.lua` `CalcTools.lua` `CalcFormat.lua` | 7,048 | **code**: `CalcTools.lua`, `CalcSetup.lua` (initEnv: tree merge, items, skills/supports, buildActiveSkillModList, radius/threshold/transform jewels, granted-skill and Explode groups, Energy Blade re-entry) and the whole `CalcPerform.lua` body are written. `Calcs.lua`'s cache half is written — `cacheData`, `buildActiveSkill`, the `buildOutput` cache fill (`calc/buildoutput.go`), `PerformFull` with the real handoff — so `GlobalCache` is computed and compared, not a fixture. Unwritten: `buildOutput`'s display half (FullDPS roll-up, cost warnings, conditions/multipliers discovery), `CalcFormat.lua`, CALCS mode, and the remaining guarded branches at the initEnv stage — `ProcessSocketGroup` nameSpec migration, `ExtraJewelFunc` re-entry, minion damage fixup — plus 5 in perform (`getCachedOutputValue` stage caches, `calcs.resistances` Choir path, non-empty `AffectedByAuraMod`). CALCULATOR mode is now written (copyActiveSkill's second initEnv); CALCS still is not. **archive**: `test/calc_test.go` — 193 variants across 64 corpus builds agree post-initEnv (`.dbs`, `.skills`, `.skillLists`) and post-perform (`.performDbs`/`.performOutput`/`.performMinion*`), plus a fixture round-trip (`TestCalcFixtureEcho`) with corruption negatives. `ReplayInput` is down to `GlobalCache` (computed by the driver; the checkpoint replay seeds it) and `StubHandoff` (fixture mode only): the three build-state fields it used to carry are derived now — anointed and Forbidden Flame/Flesh nodes through `SpecInput.Passives` (`calc.PassiveLookup`), and the Energy Blade weapon through `calc.energyBladeFor`, which builds it with the item package the way `CalcSetup` does. The dumped `energyBladeItems` records became the assertion (`assertEnergyBlades`); negative control: synthesizing the wrong hand count fails it. **Reference quirk reproduced**: the Energy Blade item gets no implicit lines, because the reference looks them up on `item.baseName` — a string, where the index is always nil. |
| `[~]` | `[~]` | `calc-offence` | `Modules/CalcOffence.lua` `CalcActiveSkill.lua` `CalcMirages.lua` `CalcTriggers.lua` | 9,145 | **code**: `CalcOffence.lua` written end to end, `radiusTertiaryBaseMargin` and `explosiveArrowFunc` included; `CalcActiveSkill.lua` landed with calc-core, plus `copyActiveSkill`. `CalcTriggers.lua`: **80 of 81 configTable entries written** (`"avenging flame"` is the unported one, see the trigger-configs row), plus `CWCHandler`, `helmetFocusHandler`, and the Arcanist Brand / Battlemage / Infernal Cry / Manaforged / Kitava mana-spent / Doom Blast-vixen handler branches. Doom Blast's expiration/hexblast sources and `stagesAreOverlaps` (a reference-dead hook no entry sets) are written too — the trigger module is complete. `CalcMirages.lua`: complete — all five paths plus the copyActiveSkill minion branch. PvP scaling (offence + EHP) written; party-tab crit stays guarded (**party is deferred by decision**, 2026-08-26 — no party tab planned). Skill callbacks: all 134 declared (granted effect, callback kind) pairs written — 133 in the `skillFuncs` registry (`calc/skillfuncs.go`) plus `explosiveArrowFunc`, which `offenceCrit` calls by granted-effect name exactly as `CalcOffence.lua` L2974 does, so it has no registry key (later.md). 132 registry pairs are reached and verified by the corpus (10 authored builds added 2026-09-03: `hittime`, `stagemult`, `spellmods`, `bow`, `wand`, `melee`, `melee2`, `unarmed`, `minion1`, `minion2`; 4 added 2026-09-05: `ragevortex`, `spectre1` — a Hydra spectre with Barrage as its main skill, which also exercises the spectre-list block `CalcPerform.lua` L2156-2221 ported the same day into `calc/performbuffs.go` — `spectre2`, a Blasphemer spectre whose `EnemyCurseLimit` lets its three curses land, and `spectre3`, a Restless Knight slam plus a Frozen Cannibal cold-resistance aura whose stats interpolate linearly on the minion level (`calc/skillmods.go`; negative control: the actor level left unset fails it); `socketless` and `socketamulet`, dumped 2026-09-04, joined the calc variant map 2026-09-05, so every dump is now a calc variant); 1 is written but unreached and so unverified: `AvengingFlame` (needs the trigger table's `"avenging flame"` entry, the one unported configTable entry). `RageVortex` reached once the dump stopped recording the offence residue `div` on its two `divVar` tags (later.md 6); `AzmeriHydraBarrage` reached once the spectre block set `HaveBeastSpectre` and the minion actor level fed skill-stat interpolation (`calc/skillmods.go`, `CalcActiveSkill.lua` L567). `Vortex` and `VortexAltX` are negative controls — their data carries no cooldown, so both sides write nothing; the shared `cooldownHitTime` body is verified through `LightningBolt`. **archive**: `test/calc_test.go` — 193 variants agree, 115 values only once quantized to 14 significant figures (37 ladder + 26 authored shells + the hand-made mjolner build; `test/corpus/authored_*.xml` exist purely to reach guards) on every stage checkpoint, no slice skipped; the 19 quantized values the 2026-09-03 builds still add are ignite-chance leaves (the `math.Pow` class, knowledge.md 4.7); their 2 base-damage leaves from a one-ulp level multiplier closed 2026-09-04 with the exporter's one-division rule, 126 -> 115 overall (4.7). Negative controls: +1e-7 on `AverageHit` fails all; +1e-9 on the simulated trigger rate fails exactly the 7 checkpoints of the two trigger-driven skills; +1 on the mirage count fails `mirage.full`. Corpus exercises: dual wield, Unleash seals, bifurcated crit, returning projectiles, bleed/poison/ignite/impale, brands, chaining, projectiles, pierce, DoT, traps, mines, ballistas, mirages, cast-when-damage-taken, cast-on-crit, cast-while-channelling |
| `[~]` | `[~]` | `calc-defence` | `Modules/CalcDefence.lua` | 3,828 | **code**: `resistances`, `defence` and `buildDefenceEstimations` written in full — resistance conversion/caps, block, primary defences, evade, suppression, dodge, regen, recharge, recoup, damage reduction, avoidance, self-ailment duration, then the EHP half (pool drains, `numberOfHitsToDie`, max hit taken, EHP vs dots, degens). PvP scaling written. The 4 party-tab branches (block/max-block/max-life-leech equal-to-party, `MovementSpeedEqualHighestLinkedPlayers`) and `TakenFromPartyMemberESBeforeYou` stay guarded — **party is deferred by decision** (2026-08-26). **archive**: `test/calc_test.go` — 193 variants agree on `.defence*` and `.ehp*` (mod/enemy/item DBs plus player and minion output, 506 output keys incl. `TotalEHP` to 13 significant digits). Negative controls: a 1e-7 perturbation inside `reducePoolsByDamage` and a one-value change in the defence tail each fail every variant |
...
| code | archive | module | files | lines | evidence & gaps |
|---|---|---|---|---|---|
| `[x]` | `[x]` | `game-data` | `Modules/Data.lua` + `Data/` (134 files) | 798,584 | `/data` package + `test/gamedata_test.go`: 136 subtrees of the loaded `data` table compared canonically against the booted archive (tools/dump_gamedata.lua), 0 disagreements — the full load surface: all inline tables, mod pools, enchantments, item bases+lists, uniques (incl. the generated ones), skills (1,535 granted effects with structured mods, statMaps, template fragments), skillStatMap, gems + lookups, minions/spectres, cluster jewels, bosses+bossSkills, mapMods, timeless tables. Residuals owned by other modules: the hand-written Lua callback bodies (declared here as `SkillCustom.Callbacks` and `MapModApplyUnported`, ported in calc/config), 4 tree-built generated uniques (→ tree-data), describeStats (→ stat-describer), LUT readers (→ timeless-jewel-data), vocab.go regeneration (small follow-up). **code** `[x]` for the tables. It also declares the reference's hand-written Lua closures without their bodies: 134 (granted effect, callback kind) pairs across `skills_custom.go` and 42 map-mod `apply` closures marked `MapModApplyUnported` in `mapmods.go`. Those bodies belong to the consuming module, not here — 34 of the skill callbacks now live in `calc/skillfuncs.go`, keyed (id, kind). (The `UnportedFn` marker type this row used to name was retired by the 2026-08-29 remodel.) |
...
| `logic:[~] view:[ ]` | `items` | `Classes/ItemsTab.lua` `Modules/ItemSlotHelper.lua` | 5,243 | each crafting popup fuses an option-list builder, a DPS-sorted ranking and an apply step — those three are logic, the dialog is view. **logic**: package `build` ports the slot table and the item-set load — 18 base slots in order, a `" Swap"` twin per weapon, six abyssal sub-slots under each weapon-swap twin and under each of the seven hosting slots, then one `"Jewel <id>"` per tree socket node ascending (131 on 3.29); `slotNum` is `tonumber(name:match("%d+$") or name:match("%d+"))`; equipped items from the active `<ItemSet>`, socket slots from `spec.jewels`; radius nodes/attributes off the spec socket node. Deliberately not reproduced: `UpdateSockets`'s `"Socket #N"` relabelling (a layout pass — every dump carries the bare `"Socket"`), and `slot.containJewelSocket`, which `CalcSetup` reads but the reference assigns nowhere, so it is always nil. Unported: crafting, `IsItemValidForSlot`, auto-equip, trade query. |
| `logic:[~] view:[ ]` | `skills` | `Classes/SkillsTab.lua` | 1,475 | socket-group processing, gem matching and `OptimiseSockets` are logic; gem slot rows are view. **logic**: package `skills` ports the load half — XML → socket groups/gems as scalar bags (attribute presence mirrors Lua: a saved "nil" string is present-and-falsey-compared), gem resolution (gemId/variantId/skillId branches, the object-keyed gemForSkill lookup, FindSkillGem nameSpec migration via calc's port + its errMsg), ProcessSocketGroup (validateGemLevel, req fields; colour codes skipped as view), default-gem-level legacy mapping, RebuildImbuedSupportBySlot, UpdateSocketGroups (socket-colour matching: a gem past the item's socket count, or on a socketless item, does not match — the reference deletes the key there, which the comparison reads as false). `test/skills_test.go`: 487 groups / 1,139 gems byte-identical across 64 builds (bool keys in `luacanon.FalseIsAbsent` render only when true on both sides; `authored_socketless.xml` puts a group on a socketless Choir of the Storm and one on a `B-R` body armour, `authored_socketamulet.xml` a group on an Unset Amulet (`Has 1 Socket`, `B`) and one on an Unset Ring (`R`), so absent, true and false are all exercised on armour and jewellery slots); masked with owners stated: calc-stamped keys (skillPart/skillMinion/stage/mine families, triggered), view keys (color), granted (`source`) groups compare XML-owned keys only (the calc's granted update rewrites their gems — ported in calc, proven by the bridge). The calc native bridge feeds calc the native tab (reduced variants: groups wiped, calc recreates granted ones; stale imbued map semantics kept): 193 variants byte-identical; negative control: one gem level +1 fails 319 comparisons. Unported: OptimiseSockets, ProcessGemLevel (new-gem defaulting; nothing on the load path calls it). |
...
| `logic:[x]` `view:[ ]` | `config` | `Classes/ConfigTab.lua` `Modules/ConfigOptions.lua` `ConfigVisibility.lua` `Classes/ConfigSetListControl.lua` | 4,179 | 580 options (check 331, count 163, countAllowZero 33, list 41, integer 11, float 1) carrying 532 apply closures. **logic**: package `config` ports the load half and `BuildModList` — the option table converted once from the booted reference (`Options`, in varList order, which is load-bearing), `NewConfigSet`'s defaults, `Load`'s `<Input>`/`<Placeholder>`/`<ConfigSet>`/`<CustomModifierBlock>` handling with the `enemyIsBoss` and `presetBossSkills` value migrations, `UpdateLevel`, the custom-modifier parse, and the three control operations the apply functions reach through (`SetPlaceholder`, `SelByValue`, `selIndex`). **All 532 apply bodies ported**: 487 converted mechanically from the reference text, the rest hand-ported with their source lines cited (`apply_hand.go`), plus `Data/ModMap.lua`'s 41 map-affix appliers, which the map dropdowns dispatch into — 4 of those are empty in the reference and registered as such. `config/coverage_test.go` holds the reference's own list and fails if an option gains or loses a body. **archive**: `test/config_test.go` — `TestConfigStateAgainstReference` compares the loaded input and placeholder tables key for key (3,635 values across 64 builds); `TestConfigModListCoverage` compares the modifier lists (**3,199 of 3,199 byte-identical, none produced that the archive lacks**). The calc differential runs on this config: `build.Load` fills all four fields and 193 variants still agree. `TestConfigOptionsAgainstReference` closes the corpus's coverage gap: `tools/dump_config.lua` sets **every** option in turn on an otherwise-default build - each value of a list option, four of a numeric one - and records what BuildModList produced; the test replays all 1,254 cases. **73,824 comparisons across all 580 options, zero disagreements.** Negative control: flipping one FLAG's value fails it. **Traps**: `SetPlaceholder(v, true)` round-trips through the control's `tostring`/`tonumber`, so a stored placeholder is quantized; `SetPlaceholder("", true)` DELETES the key; a string-valued `<Placeholder>` is stored as an *input*; and `mapAffixDropDownFunction` passes an undeclared global as the check-type appliers' first argument (a reference quirk, `later.md` §2). Unported: `ConfigVisibility.lua` (per-option relevance — view), the widgets, undo, config-set management. |
...
(`test/luarender/luaprng.go`) and hash-table iteration
(`test/luarender/luatab.go`) replicas (both baked
into *data*: LegionPassives offsets, tradeHashes order — both now under
`test/luarender/`), and 21 of 24 `Scripts/*.lua` as document builders
(`enums.lua` became `export.WriteEnumFiles`; `uTextToMods.lua` is a no-op in
the reference). Extraction stays with
...
`Data/*.lua` byte format with `test/luarender` (test-only; holds all the
