...
| S12 | `calc/setup.go:164-172`, `calc/tools.go:409-419` | `AllocOrders/ExtraOrders` — `pairs()` order captured from the reference dump and replayed; panics at `calc/setup.go:517,542,556,561` when misaligned | ~~Load-bearing (float sums are not associative; LuaJIT hash order not derivable)~~ **Corrected 2026-08-30**: the dump ran the Calc modules under `sortedPairs` (`tools/dump_calc.lua:131`), so the captured orders are ascending node ids — derivable by sorting. Replaced by sorted iteration; see `lua-residue.md` T1 |
...
| L10 | captured `pairs()` order replay `AllocOrders/ExtraOrders` (S12) | ~~float sums over LuaJIT hash order~~ **Corrected 2026-08-30**: the order is sorted node ids (`tools/dump_calc.lua:131` — `pairs = sortedPairs`); replay removed for sorted iteration (`lua-residue.md` T1) | — |
